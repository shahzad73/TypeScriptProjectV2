...
import { IUpdates } from '../../entities/Updates';
...
export default function Home() {
    ...
    const [inputs, setInputs] = useState({} as IUpdates);
    ...
    React.useEffect(() => {   
        ...
        if(location.state.update == 1) {
            ...
            axios.get("/platform/backend/getUpdate?id=" + id).then(response => {
                ...
                setStartDate (  new Date(  moment(response.data[0].UpdateDate).format('MMMM DD, YYYY')  ).toString()  );
            }).catch(function(error) {
                console.log(error);
            });     
        }
